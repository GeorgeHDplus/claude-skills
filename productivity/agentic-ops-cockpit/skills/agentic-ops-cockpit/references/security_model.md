...
| Replay eines abgefangenen Handler-Requests | 90-s-Zeitfenster + TLS im Tunnel; Confirms zusätzlich: KV-Eintrag wird **vor** Ausführung gelöscht (minimiert das Doppel-Fenster). KV ist nicht atomar — strikte Exactly-once bräuchte ein Durable Object; praktisch abgesichert durch idempotente Aktionen (s.u.) |
...
- Keine harte Exactly-once-Garantie auf dem Confirm-Flow: Cloudflare KV ist eventual consistent, `get`→`delete` nicht atomar. Das Fenster für eine Doppelausführung ist durch Löschen-vor-Ausführung minimal, und abgesichert durch das **Idempotenz-Prinzip** — jede Aktion zweifach ausgeführt richtet nicht mehr Schaden an als einfach (für `pc.run_script` heißt das: die gestarteten Skripte sind idempotent, Georges Standard). Strikte Exactly-once wäre mit einem Durable Object erreichbar (Roadmap-Option, bewusst nicht Default für dieses Single-User-Setup).
